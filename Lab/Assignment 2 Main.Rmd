---
title: "Assignment 2"
author: "Jatin Jaitley Student No: 18200075"
...
cat("Proportion of the lawyers who practice litigation")
cat(format(round(proportion, 2), nsmall = 2))
```
###Question 1 Part 2
```{r}
...
if (Proportion_Boston > Proportion_Providence) {
  cat('Proportion of lawyers in Boston office that practice corporate law is high which =',Proportion_Boston)
}else
  cat('Proportion of lawyers in Providence office that practice corporate law is high which =',Proportion_Providence)
```
###Question 1 Part 3
```{r}
...
```
###Question 2 Part 3 Function timing check 
```{r}
...
###Interpretation
```{r}
cat("We observed that average rain in Cork is more than the average rainfall in Dublin. Also the temperature range of more in Dublin.")
cat("\n On the Basis of the above observation, we can say that Dublin is warmer than Cork.")
```

###Question 3 Part 4
```{r}
plot.WeatherData<-function(dtf,maxi_color1="red",mini_color2="blue",bar_high="red",title="Daily Temperature & Precipitation Graph",is_pt_req=TRUE){
  #print(dtf)
par(mfrow = c(2,1))
plot(dtf$maxtp,type='o',pch=16,ylim=c(0,25),col = maxi_color1,xlab = "Dates(Sep-2018)",ylab =" Air Temperatures(C)",main=title) # add the red line
...
if(is_pt_req==TRUE){
   ...
   text(min_date,min(dtf$mintp),label=min(dtf$mintp))
}
# Add in the legend
legend('topright', legend = c('MaxTmp', 'MinTmp'), lty=c(1,1), cex=.5, 
col = c('red', 'blue'))
maxfac <- 1+(dtf$rai==max(dtf$rain))
barplot(names.arg=dtf$date, dtf$rain,ylim=c(0,25),las = 1,xlab = 'Dates(Sep-2018)',col=c("blue",bar_high)[maxfac],  ylab = "PPT Amt (mm)")
#axis(1, 1:30, labels = TRUE)
#axis(1, at=seq(1,30,1))
}
plot(DublinAirport,maxi_color1 = "red",mini_color2="blue",bar_high = "red",title="DayWise Temperature & Precipitation Dublin",is_pt_req = FALSE)
plot(CorkAirport,maxi_color1 = "orange",mini_color2="black",bar_high = "orange",title="Daily Temperature & Precipitation Cork",is_pt_req = FALSE)
plot(CorkAirport)
plot(CorkAirport,'black','yellow','green',title="DayWise Temperature & Precipitation Cork")
```
